...
type
  ...
  private
    ...
    _SCardAuth:function(hCard: DWORD;ulMifareBlockNr: DWORD;ucMifareAuthMode: byte; ucMifareAccessType: byte;ucMifareKeyNr: byte;out pucMifareKey: byte; ulMifareKeyLen: DWORD): DWORD; stdcall;
    _SCardRead:function(hcard :SCARDHANDLE ;
                         ulMifareBlockNr :ULONG ;
                         out inBuffer : PUCHAR;
                         ulMifareDataReadBufLen: ULONG ;
                         out pulMifareNumOfDataRead :ULONG):ULONG; stdcall;
    _SCardWrite:function(hCard: SCARDHANDLE;
                           ulMifareBlockNr: ULONG;
                           out pucMifareDataWrite: PUCHAR;
                           ulMifareDataWriteBufLen: ULONG): ULONG; stdcall;

    _SCardCLGetUID:function(hcard:DWORD;
          out pucUID:  UCHAR;
          ulUIDBufLen: DWORD;
          out pulnByteUID: DWORD ): DWORD; stdcall;
  ...
  public
    ...
    function SCardStatus(hCard: DWord; pReaderNames: PChar;var SizeReaderNames, State, Protocol: DWord;pAtr: Pointer; var SizeAtr: DWord): DWord;
     ...
     function SCardAuth(
            ...
            ulMifareKeyLen: DWORD): DWORD;

  function SCardRead(hcard :DWORD;
            ulMifareBlockNr :DWORD ;
            out inBuffer : PUCHAR;
            ulMifareDataReadBufLen: DWORD ;
            out pulMifareNumOfDataRead :DWORD):DWORD;

  function SCardWrite(hCard: SCARDHANDLE;
            ulMifareBlockNr: ULONG;
            out pucMifareDataWrite: PUCHAR;
            ulMifareDataWriteBufLen: ULONG): ULONG;

   function SCardCLGetUID(hcard:DWORD;
          out pucUID:  UCHAR;
          ulUIDBufLen: DWORD;
          out pulnByteUID: DWORD ): DWORD;
...
function TPCSCRaw.SCardAuth(hCard, ulMifareBlockNr: DWORD;
  ucMifareAuthMode, ucMifareAccessType, ucMifareKeyNr: byte;
  out pucMifareKey: byte; ulMifareKeyLen: DWORD): DWORD;
begin
    Result:=_SCardAuth(hCard,ulMifareBlockNr,
    ucMifareAuthMode, //uth mode 96=
    ucMifareAccessType, //access option 0 = key B  1=Key A Number
    ucMifareKeyNr, //mifareKeyNr
    pucMifareKey,
    ulMifareKeyLen);
end;

function TPCSCRaw.SCardRead(hcard:DWORD;
  ulMifareBlockNr :DWORD ;
  out inBuffer : PUCHAR;
  ulMifareDataReadBufLen: DWORD ;
  out pulMifareNumOfDataRead :DWORD):DWORD;
...
begin
    Result:=_SCardRead(hcard,ulMifareBlockNr,inBuffer,ulMifareDataReadBufLen,pulMifareNumOfDataRead);
end;

function TPCSCRaw.SCardWrite(hcard:DWORD;
  ulMifareBlockNr: ULONG;
  out pucMifareDataWrite: PUCHAR;
  ulMifareDataWriteBufLen: ULONG): ULONG;
begin
    Result:=_SCardWrite(hcard,ulMifareBlockNr,pucMifareDataWrite,ulMifareDataWriteBufLen);
end;

function TPCSCRaw.SCardCLGetUID(hcard: DWORD; out pucUID: UCHAR;
  ulUIDBufLen: DWORD; out pulnByteUID: DWORD): DWORD;
begin
     Result:=_SCardCLGetUID(hcard,pucUID,ulUIDBufLen,pulnByteUID);
end;
